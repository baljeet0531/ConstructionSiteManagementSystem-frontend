...
export default function SitePage() {
    ...
    const allSites = MOCK_SITE.allsites.map((siteDetails, index) => {
        return <Site key={index} siteDetails={siteDetails} handlePopup={handlePopup}></Site>
    })
    ...
    return (
        <Box>
            <Flex direction={"column"} w={"80vw"} h={"100vh"} pl={"30px"} pr={"30px"} pt={"47px"} pb={"20px"} overflowY={"auto"}>
                <Flex direction={"row"} justify="space-between" align={"end"} mb={"5px"}>
                    ...
                    <Spacer />
                    <Button leftIcon={<AddIcon />} bg={"#4C7DE7"} color={"#FFFFFF"} onClick={() => (handlePopup("addSite"))}>新增工地</Button>
                </Flex>
